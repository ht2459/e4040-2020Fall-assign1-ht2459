...
class TwoLayerNet(object):
    ...
    def loss(self, X, y):
        ...
        # TODO: Feedforward                               #
        ###################################################
        W1, b1, W2, b2 = self.layer1.params[0], self.layer1.params[1], self.layer2.params[0], self.layer2.params[1]
        X = X.reshape(N, self.layer1.input_dim)
        ...
        ###################################################      
        dX1, dW2, db2 = affine_backward(dscores, hidden_layer, W2, b2)
        dW2 += self.reg * W2

        r_b = relu_backward(dX1, hidden_layer)
        dX, dW1, db1 = affine_backward(r_b, X, W1, b1)
        dW1 += self.reg * W1

        self.layer1.gradients = [dW1, db1]
        self.layer2.gradients = [dW2, db2]        
    ...
    def predict(self, X):
        ...
        #            START OF YOUR CODE                   # 
        ###################################################  
        predictions = np.argmax((X.dot(layer1.params[0])).dot(layer2.params[0]),axis=1)
